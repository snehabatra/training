  ...
  console.log("reached");
            var dataset;
            var DataType;
           var createStatement = "CREATE TABLE IF NOT EXISTS sneha(id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT, password TEXT, op1 TEXT , op2 TEXT, address TEXT,remember TEXT )";
            var insertStatement = "INSERT INTO sneha(email, password,op1,op2,address,remember) VALUES (?,?,?,?,?,?)";
            var updateStatement = "UPDATE sneha SET email = ?, password = ?, op1=?, op2=?, address=?, remember=? WHERE id=?";
...
var html;
            ...
            html = html + '      <th>male</td>';
            html = html + '      <th>female</td>';
            ...
            html = html + '      <td>' + item['op1'] + '</td>';
            html = html + '      <td>' + item['op2'] + '</td>';
